...
export function DashboardContainer({ userName }: { userName?: string | null }) {
    ...
    <main className="flex-1 p-6 md:p-10 max-w-7xl mx-auto w-full">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 font-medium">
              Gestión Fiscal y Administrativa · Clifford Standard
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="text-right hidden md:block">
              ...
              <button
                onClick={() => signOut()}
                className="text-[10px] font-bold text-gray-400 hover:text-status-error uppercase tracking-tighter transition-colors"
              >
                Cerrar Sesión
              </button>
            </div>
            <button
              onClick={() => signOut()}
              className="md:hidden p-2 text-gray-400 hover:text-status-error transition-colors"
            >
              <LogOut size={20} />
            </button>
          </div>
      ...
      <footer className="mt-12 pt-6 border-t border-gray-100 text-center">
        <p className="text-xs text-gray-400 font-medium">
          © 2026 Clifford Standard Spoke · Orquestador Alfredo · Hub & Spokes
          Ecosystem
        </p>
      </footer>
    </main>
